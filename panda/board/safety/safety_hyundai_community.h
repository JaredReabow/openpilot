...
const CanMsg HYUNDAI_COMMUNITY_TX_MSGS[] = {
  ...
  {790, 1, 8}, // EMS11, Bus 1 could be 790
...
static int hyundai_community_fwd_hook(int bus_num, CAN_FIFOMailBox_TypeDef *to_fwd) {
  ...
  if ((HKG_forward_bus1 || HKG_forward_obd) && (addr == 688 ||addr == 608 || addr == 1136 || addr == 870 || addr == 790)){fwd_to_bus1 = 1;} //MDPS SPAS
  // forward cam to ccan and viceversa, except lkas cmd
  if (HKG_forward_bus2) {
    if (bus_num == 0) {
      if (!OP_CLU_live || addr != 1265 || HKG_mdps_bus == 0) {
        if (!OP_MDPS_live || addr != 593) {
          if (!OP_EMS_live || addr != 790) { // Could be 790 for ems11
